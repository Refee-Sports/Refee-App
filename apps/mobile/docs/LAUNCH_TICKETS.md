...
| SEC-02 | BLOCKED — EXTERNAL | Set STRIPE_WEBHOOK_SECRET in production | The secret is not set on the hosted project, so stripe-webhook answers every Stripe delivery with 400 "Webhook is not configured" — no payment event is being processed live. Copy the signing secret from the Stripe dashboard's live webhook endpoint into Supabase → Edge Functions → Secrets. Blocks PAY-01. |
